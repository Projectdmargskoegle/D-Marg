  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch(
        `http://localhost:8100/find?fromdate=${filter?.fromDate}&todate=${filter?.toDate}&fromtime=${filter?.fromTime}&totime=${filter?.toTime||"23:00:00"}&deviceName=${filter?.selectedDevice}`
      );
      if (!response.ok) {
